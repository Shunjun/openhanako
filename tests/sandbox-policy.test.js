...
describe("sandbox workspace roots", () => {
  ...
  it("merges customWritablePaths into writablePaths", () => {
    const agentDir = path.join(tempRoot, "agents", "hana");
    const hanakoHome = path.join(tempRoot, "home");
    const workspace = path.join(tempRoot, "project");
    const customDir = path.join(tempRoot, "custom-data");
    for (const dir of [agentDir, hanakoHome, workspace, customDir]) {
      fs.mkdirSync(dir, { recursive: true });
    }
    ...
    const policy = deriveSandboxPolicy({
      ...
      workspaceFolders: [],
      mode: "standard",
      customWritablePaths: [customDir],
    });

    expect(policy.writablePaths).toContain(workspace);
    expect(policy.writablePaths).toContain(customDir);
    expect(policy.writablePaths).toContain(path.join(agentDir, "memory"));
  });

  it("deduplicates customWritablePaths that overlap with defaults", () => {
    const agentDir = path.join(tempRoot, "agents", "hana");
    const hanakoHome = path.join(tempRoot, "home");
    const workspace = path.join(tempRoot, "project");
    ...
    const policy = deriveSandboxPolicy({
      ...
      workspaceFolders: [],
      mode: "standard",
      customWritablePaths: [workspace],
    });

    const occurrences = policy.writablePaths.filter((p) => p === workspace);
    expect(occurrences).toHaveLength(1);
  });

  it("ignores relative paths in customWritablePaths", () => {
    const agentDir = path.join(tempRoot, "agents", "hana");
    const hanakoHome = path.join(tempRoot, "home");
    const workspace = path.join(tempRoot, "project");
    for (const dir of [agentDir, hanakoHome, workspace]) {
    ...
    const policy = deriveSandboxPolicy({
      ...
      workspaceFolders: [],
      mode: "standard",
      customWritablePaths: ["relative/path", ""],
    });

    expect(policy.writablePaths).not.toContain("relative/path");
    expect(policy.writablePaths).not.toContain("");
  });
});
