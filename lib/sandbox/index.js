 ...
 * @param {() => string[]} [opts.getSandboxWritablePaths]  用户自定义的额外可写路径
...
export function createSandboxedTools(cwd, customTools, {
  ...
  const customWritable = typeof getSandboxWritablePaths === "function" ? getSandboxWritablePaths() : [];
  const policy = deriveSandboxPolicy({ agentDir, workspace, workspaceFolders, hanakoHome, mode: "standard", customWritablePaths: customWritable });
  ...
  const readTool = wrapReadImageWithVisionBridge(createReadTool(cwd, { operations: readOps }), cwd, {
    getSessionPath,
    recordFileOperation,
    getVisionBridge,
    isVisionAuxiliaryEnabled,
