...
export class DeferredResultCoordinator {
  ...
  enqueueTask(taskId, task) {
    ...
    if (bucket.lastSeenState !== "active_streaming") {
  ...
  _getSessionState(sessionPath) {
    ...
    if (binding?.pi && session?.isStreaming) return "active_streaming";
    if (binding?.pi) return "active_idle";
    return "detached";
  }
  ...
  async _flushSession(sessionPath) {
    ...
    try {
      while (bucket.items.size > 0) {
        ...
        if (bucket.lastSeenState === "active_streaming") break;

        let taskIds = null;
        const tasks = Array.from(bucket.items.values());

        if (bucket.lastSeenState === "detached") {
      ...
      } else if (bucket.needsFlush || this._getSessionState(sessionPath) !== "active_streaming") {
        this._requestFlush(sessionPath);
      }
    }
  }
